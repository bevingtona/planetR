...
planet_order <- function(api_key, bbox, date_start, date_end, start_doy, end_doy,
                         cloud_lim, item_name, product, order_name = exportfolder){
  ...
  items_all = response[,1]
  items_groups <- split(items_all, ceiling(seq_along(items_all)/100))

  for(i in 1:length(items_groups)){
    print(paste0("Group #",i," (<100 images per group)"))
    items <- items_groups[i][[1]]

    products = list(list(item_ids = items,
    ...
    if(!is.null(post_content$field$Details[[1]]$message)){
      print(post_content$field$Details[[1]]$message)}else{
  ...
  planet_order_download <- function(order_id, order_name){
    ...
    for(i in 1:length(get_content$`_links`$results)){
      print(paste0("Download: ", signif(100*(i/length(get_content$`_links`$results)), 1),"%"))
      ...
      httr::RETRY("GET",
            url = download_url,
            username = api_key,
            write_disk(path = paste(order_name, filename, sep = "/"),
                       overwrite = TRUE))
